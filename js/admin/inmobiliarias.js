...
async function render() {
    const users = (await SkalaDatabase.get('users')).filter((u) => u.role === 'inmobiliaria');
    const box = document.querySelector('[data-admin-inmobiliarias]');

    box.innerHTML = users.length ? table(users) : '<div class="empty">No hay inmobiliarias registradas.</div>';
}

function table(users) {
    return `<table class="table"><thead><tr><th>Nombre</th><th>Correo</th><th>Empresa</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>${users.map((u) => `
        <tr>
            <td>${SkalaUI.safe(u.name)}</td>
            <td>${SkalaUI.safe(u.email)}</td>
            <td>${SkalaUI.safe(u.company)}</td>
...
async function editUser(id) {
    ...
    if (!name) return;
    await SkalaDatabase.update('users', id, { name });
    render();
}
